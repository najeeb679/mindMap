...
// No need for `window` prop if it's not being used.
...
const navItems = [
  ...
  { title: "LogIn", path: "/logIn" }, // Ensure LogIn has a valid path
];
      ...
      <List>
        {navItems.map((item) => (
          <ListItem key={item.title} disablePadding>
            <ListItemText style={{ textAlign: "center" }} primary={item.title} />
          </ListItem>
        ))}
      </List>
    </Box>
          ...
          >
            ...
            >
              ...
              <Typography whiteSpace={"nowrap"}>
                ...
                <br />
                <span style={{ fontWeight: "300", fontSize: "14px" }}>Reiki Master</span>
              </Typography>
            </NavLink>
          </Box>

          <Box sx={{ display: { xs: "none", sm: "block" }, textWrap: "nowrap" }}>
            {navItems.map((item) => (
              <NavLink
                to={item.path}
                key={item.title}
